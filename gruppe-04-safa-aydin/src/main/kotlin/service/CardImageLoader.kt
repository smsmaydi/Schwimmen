...
class CardImageLoader {
    ...
    /**
     * The full raster image containing the types as rows (plus one special row for blank/back)
     * and values as columns (starting with the ace). As the ordering does not correctly reflect
     * the order in which the types are declared in [CardType], mappings via [row] and [column]
     * are required.
     */
    private val image : BufferedImage = ImageIO.read(CardImageLoader::class.java.getResource(CARDS_FILE))

    /**
     * Provides the card image for the given [CardType] and [CardValue]
     */
    fun frontImageFor(type: CardType, value: CardValue) =
        getImageByCoordinates(value.column, type.row)
...
/**
 * As the [CARDS_FILE] does not have the same ordering of types
 * as they are in [CardType], this extension property provides
 * a corresponding mapping to be used when addressing the row.
 *
 */
private val CardType.row get() = when (this) {
    CardType.CLUB -> 0
    CardType.DIAMOND -> 1
    CardType.HEART -> 2
    CardType.SPADE -> 3
